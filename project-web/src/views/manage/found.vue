<template>
  <el-main>
    ...
    <el-table :data="tableList" :height="TableHeight" style="width: 100%">
      ...
      <el-table-column prop="lfUsername" label="拾获者" width="100"></el-table-column>
      ...
      <el-table-column prop="lfTime" label="拾获时间" width="120"></el-table-column>
      ...
      <el-table-column prop="isFound" label="招领状态"  width="100">
        <template #default="scope">
          <el-tag v-if="scope.row.isFound == '0'" type="danger" size="default" effect="Light">
            待领回
          </el-tag>
...
import {isPassFoundList,foundAdminStatus,foundAdminDel} from '@/api/found';
import { likAll } from '@/api/ilk';
import { FoundStatus } from '@/api/found/FoundModel';
...
//获取表格的数据
const getList = async () => {
  let res = await isPassFoundList(searchParm);
  if (res && res.code == 200) {
...
const commit = () => {
  ...
  addForm.value?.validate(async (valid) => {
    if (valid) {
      let res  = await foundAdminStatus(addModel);
      if (res && res.code == 200) {
...
const deleteBtn = async (lfId: string) => {
  ...
  console.log(confirm)
  if (confirm) {
    let res = await foundAdminDel(lfId)
    if (res && res.code == 200) {
      ElMessage.success(res.msg)
      //刷新列表
      getList()
